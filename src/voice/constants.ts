/** Timings for the spoken loop, from published guidance rather than taste: a 200-450 ms gap reads
 * as natural, a barge-in must land under 150 ms. */
...
/** Two thresholds, not one: rising through SPEECH_ON opens a turn and falling below SPEECH_OFF
 * closes it, so a wavering voice cannot rattle the machine. */
...
/** How long to wait, after the voice stops, before the turn is handed to the model. The turn
 * detector picks one of these from what was actually said. */
export const ENDPOINT_FAST_MS = 460;
export const ENDPOINT_BASE_MS = 700;
export const ENDPOINT_SLOW_MS = 1150;

/** Transcription starts here, not at the endpoint. A drawn breath throws the result away; a
 * finished sentence is already transcribed when the turn ends. */
...
/** A partial pass is skipped when the last took longer than this, so a slow machine falls back to
 * one pass instead of building an unclearable backlog. */
...
/** The microphone still hears the assistant through echo-cancellation residue, so a higher
 * probability during output keeps it from interrupting itself. */
